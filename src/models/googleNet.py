...
class GoogleNet(object):
    ...
    def __create_model(self) -> None:
        ...
        layer = MaxPooling2D(pool_size=(3, 3), strides=2, padding='same')(layer)
        layer = self.__inception_layer(layer, self.inception_4a)

        softmax_0 = self.__softmax_layer(layer, "softmax_0")

        layer = self.__inception_layer(layer, self.inception_4b)
        layer = self.__inception_layer(layer, self.inception_4c)
        layer = self.__inception_layer(layer, self.inception_4d)

        softmax_1 = self.__softmax_layer(layer, "softmax_1")
        layer = self.__inception_layer(layer, self.inception_4e)

        layer = MaxPooling2D(pool_size=(3, 3), strides=2, padding='same')(layer)
        layer = self.__inception_layer(layer, self.inception_5a)
        layer = self.__inception_layer(layer, self.inception_5b)
    ...
    def __softmax_layer(self, prev_layer, name):
